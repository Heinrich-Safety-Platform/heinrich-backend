...
>
> - **현재 (과제 데모)**: 프론트엔드 웹앱(초기 설계 계획은 PWA 웹앱이었으나 후속 과제로 분리)의 요청에 따라 하인리히 분석 레이어 좌표 데이터를 실시간 가공하여 제공.
...
1. **[Ingestion]** 시민이 위험 현장의 QR 코드를 스캔하여 경량 웹 페이지(초기 설계 계획은 PWA 웹앱이었으나 후속 과제로 분리)를 통해 이상 징후 제보 데이터 입력.
...
- API: https://heinrich-backend.onrender.com
- API 문서(Swagger UI): https://heinrich-backend.onrender.com/docs
